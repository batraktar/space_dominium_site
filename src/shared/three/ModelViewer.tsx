...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    g.traverse((obj) => {
      ...
      if (fadeIn) setMeshOpacity(obj, 0)
    })
  ...
  }, [camera, gl, enableManualRotation, enableManualZoom, minZoom, maxZoom])
...
type ModelContentProps = { url: string } & Omit<ModelInnerProps, 'content'>

const GltfContent: FC<ModelContentProps> = ({ url, ...innerProps }) => {
  const { scene } = useGLTF(url)
  const content = useMemo(() => scene.clone(), [scene])
  return <ModelInner {...innerProps} content={content} />
}

const FbxContent: FC<ModelContentProps> = ({ url, ...innerProps }) => {
  const group = useFBX(url)
  const content = useMemo(() => group.clone(), [group])
  return <ModelInner {...innerProps} content={content} />
}

const ObjContent: FC<ModelContentProps> = ({ url, ...innerProps }) => {
  const obj = useLoader(OBJLoader, url)
  const content = useMemo(() => obj.clone(), [obj])
  return <ModelInner {...innerProps} content={content} />
}

const ModelContent: FC<ModelContentProps> = ({ url, ...innerProps }) => {
  const ext = useMemo(() => url.split('.').pop()?.toLowerCase(), [url])

  if (ext === 'glb' || ext === 'gltf') return <GltfContent url={url} {...innerProps} />
  if (ext === 'fbx') return <FbxContent url={url} {...innerProps} />
  if (ext === 'obj') return <ObjContent url={url} {...innerProps} />
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  useEffect(() => {
    ...
    if (ext === 'glb' || ext === 'gltf') void useGLTF.preload(url)
  }, [url])
  ...
  const capture = () => {
    ...
    s.traverse((obj) => {
      ...
      if (!('castShadow' in obj)) return
      const light = obj as THREE.Light & { castShadow: boolean }
  ...
  const environmentPresetValue = environmentPreset === 'none' ? undefined : environmentPreset
  ...
  return (
    ...
    >
      ...
      >
        {environmentPresetValue && (
          <Environment preset={environmentPresetValue} background={false} />
        )}
